...
### CR1-TSK-9 Manual Verification Finding
Running the app locally (`mvn spring-boot:run`) after merge failed on startup: Hibernate's `ddl-auto=update` tried `ALTER TABLE users ADD COLUMN address_line1 ... NOT NULL` against the *existing* local `backend/data/addressbook.mv.db` (which already had a seeded Admin row from prior sessions with no value for the new columns), causing a `NOT NULL` constraint violation. The Architecture.md v1.1 note claiming "no migration needed" was wrong — it's only true for a fresh database, not an existing one being altered in place. **Fix:** deleted the stale local `backend/data/*.db` files (git-ignored, disposable test data) so the schema regenerates from empty; app then started cleanly. **Not a durable fix** — flagged in Architecture.md as a real gap (`ddl-auto=update` can't safely add `NOT NULL` columns to populated tables) that a real migration tool would resolve, out of scope for this CR.
